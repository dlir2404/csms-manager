...
import { Col, Image, Table, TableProps, Tag } from 'antd'
    ...
    return (
      <>
        ...
        <div className="flex justify-between items-center">
          ...
          {paymentMethod === PaymentMethod.CASH && <div>CASH</div>}
          {paymentMethod === PaymentMethod.VNPAY && <div>VNPay</div>}
        </div>
      </>
    )
  ...
  const columns: TableProps<any>['columns'] = [
    {
      ...
      key: 'key',
      render: (value) => {
        if (value === 'Total') return <p className="font-bold text-ls">{value}</p>
        return <p>{value}</p>
      },
    },
    {
    ...
    },
    {
      ...
      render: (_, record) => {
        if (record.key === 'Total')
          return <p className="text-lg font-bold">{formatCurrency(record.price)} VND</p>
