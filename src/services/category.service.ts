...
export const useGetListCategory = ({ page, pageSize }: { page?: number; pageSize?: number }) => {
  return useQuery({
    queryKey: [QueryKey.GET_CATEGORIES, page, pageSize],
    queryFn: async () => {
...
export const useCreateCategory = (onOk?: (data: any) => void) => {
...
export const useEditCategory = (onOk?: (data: any) => void) => {
  return useBaseMutation({
...
export const useDeleteCategory = (onOk?: (data: any) => void) => {
  return useBaseMutation({
    mutationFn: async (body: any) => {
      const { id } = body
