...
import { useGetQuantityStatisticByProduct } from '@/services/order.service';

const QuantityByProductLineChart = () => {
    const [day, setDay] = useState<dayjs.Dayjs>(dayjs())

    const { data } = useGetQuantityStatisticByProduct({
        month: day.month() + 1,
        year: day.year(),
    })
...
export default QuantityByProductLineChart;